...
def calculate_demographic_data(print_data=True):
    ...
    total_rows = len(df.loc[:, 'education'])
    bachelors_rows = len(df.loc[df['education'] == 'Bachelors'])
    percentage_bachelors = round((bachelors_rows/total_rows) * 100, 1)
    ...
    over50KCountryCount = df.loc[df['salary'] == '>50K', 'native-country'].value_counts()
    percentRich = over50KCountryCount/df['native-country'].value_counts()

    highest_earning_country = percentRich[percentRich == percentRich.max()].index[0]
    
  
    highest_earning_country_percentage = round(percentRich.max() * 100, 1)

    # Identify the most popular occupation for those who earn >50K in India.

    occupationCountIndia = df.loc[(df['salary'] == '>50K') & (df['native-country'] == 'India'), 'occupation'].value_counts()
  
    top_IN_occupation = occupationCountIndia[occupationCountIndia == occupationCountIndia.max()].index[0]
